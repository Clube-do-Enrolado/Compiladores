...
def strspl(s):
   ...
   for i, char in enumerate(s): # Para cada caractere da string (contando também seu index)
        if char == WHITE_SPACE and (is_string == True):
            lexeme += char
        ...
        if char in special and (is_string == False): # Caso o char atual seja um operador
        ...
        if (i+1) < len(s):
            ...
            if (s[i+1] == WHITE_SPACE or s[i+1] in special) and (is_string == False):
